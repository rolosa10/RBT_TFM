...
#NN initialization 
model = keras.models.load_model('../../solutions/LSTM/models/Non_Gravity/'+experiment+'/model.h5')
scaler_2D = pickle.load(open('../../solutions/LSTM/models/Non_Gravity/'+experiment+'/scaler_2D.pkl','rb'))
scaler_3D = pickle.load(open('../../solutions/LSTM/models/Non_Gravity/'+experiment+'/scaler_3D.pkl','rb'))
    ...
    if cap.isOpened():
        for i in range(0,len(markers_pred_coordinates)):
            try:  
                ...
                for l in range(0,5):
                    print('Window Pict nubmer: '+str(i))
                    #MediaPipe inference
                    ...
                    window_2D_detections[l] = detected_keypoints_correct_order
                    
                df = pd.DataFrame(window_2D_detections)
                ##NN Inference
                X = np.flip(np.array(df), axis=0).astype(float)
                ...
                z_predicted_splitted = split_3_coordinates_array(z_predicted[0][4])
